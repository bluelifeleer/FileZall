...
def create_remote_client(
    protocol: Protocol,
    agent_client_factory: Callable[[], RemoteFileClient] | None = None,
) -> RemoteFileClient:
    if protocol is Protocol.SFTP:
        return SftpAdapter()
    if protocol in {Protocol.FTP, Protocol.FTPS}:
        return FtpAdapter(protocol=protocol)
    if protocol is Protocol.AGENT_HTTP and agent_client_factory is not None:
        return agent_client_factory()
    raise RemoteConnectionError("Agent HTTP requires an Agent client factory")
